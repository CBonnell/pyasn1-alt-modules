...
class Evaluation(univ.Sequence):
    componentType = namedtype.NamedTypes(
        namedtype.OptionalNamedType(
            "parameters",
            univ.SequenceOf(componentType=Parameter()).subtype(
                implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0)
            ),
        ),
        namedtype.NamedType(
...
class Algorithm(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType("evaluations", univ.SequenceOf(componentType=Evaluation())),
        namedtype.OptionalNamedType(
            "information",
...
class TBSPolicy(univ.Sequence):
    componentType = namedtype.NamedTypes(
        namedtype.DefaultedNamedType(
            "version",
            univ.Integer(namedValues=namedval.NamedValues(("v1", 1))).subtype(
                value="v1"
            ),
        ),
        namedtype.DefaultedNamedType("language", char.UTF8String().subtype(value="en")),
        ...
        namedtype.NamedType("algorithms", univ.SequenceOf(componentType=Algorithm())),
    )
